function [ate] = g2oPerformanceATEfromMATtimeStamp(GTFileName,AUTFileName, gtMapAutPose)
...
%       gtMapAutPose: vector with the length of the vertices of AUT file.
%       Has the corresponding poses to relate to that of the GT file.
%-------------------------------------------------------------------------------
%   Output:
...
for i = 1:autvCount
   % -- Find pose x and y
   gtPoseIdx = gtMapAutPose(i);
   if (gtPoseIdx > gtvCount)
       gtPoseIdx = gtvCount;
   end
   vgtx = gtVertices(gtPoseIdx).x;
   vgty = gtVertices(gtPoseIdx).y;
%
