function g2oPerformanceATEgroupG2O(filesCreatedFile, inputDirPath, GTFileName, freqDiff, outFileName)
...
for i = 1:length(matfe)
    ...
    if (matfe(i) == 1)
        fprintf('From Mat!\n');
        ateVal = g2oPerformanceATEfromMAT(GTMatFileName,AUTMatFileName, freqDiff);
    else
        fprintf('From G2o!\n');
        ateVal = g2oPerformanceATEfromG2O(AUTFileName, GTG2oFileName, freqDiff);
    end
    fprintf(fileID, '%f\n',ateVal);
    fprintf(1,'fileName: %s\n',char(fileNames(i)));
